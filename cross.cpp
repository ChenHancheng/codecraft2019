      ...
      for(int k=0; k<roads[dispatch_seq[i]].channel; k++){
        ...
        if(car_id != -1 && cars[car_id].state==WAIT && cars[car_id].next_road_id == -1){
          ...
          for(int l=0; l<valid_roads_num; l++){
            ...
            int cost;
            int direction_tmp;
            int cross_id = roads[next_road_id_tmp].start==id?roads[next_road_id_tmp].end:roads[next_road_id_tmp].start;
            int ratio;
            if(roads[next_road_id_tmp].start == id){
              direction_tmp = 0;
              ratio = (8*roads[next_road_id_tmp].cars_num_road0/roads[next_road_id_tmp].max_car_num);
            }
            else{
              direction_tmp = 1;
              if(roads[next_road_id_tmp].bidirectional == false) {
                // std::cout<<"tasdfk"<<std::endl;
                continue;
              }
              ratio = (8*roads[next_road_id_tmp].cars_num_road1/roads[next_road_id_tmp].max_car_num);
            }
            cost = roads[next_road_id_tmp].QueryRoadState(cars, car_id, direction_tmp) + cost_matrix[cross_id][cars[car_id].end]/cars[car_id].speed+ratio;
            if(cost < min_cost){
              next_road_id = next_road_id_tmp;
              min_cost = cost;
            }
          }
